...
import { planCommand, previewCommand, publishCommand } from "./commands/publish.js";
...
program
  ...
  .option("--platform <platform>", "x, linkedin, reddit, telegram, aegea, bluesky, mastodon, discord, or threads")
...
program
  .command("accounts")
  .description("Print configured social accounts.")
  .action(() => run(() => accountsCommand()));

program
  .command("account:set")
  .description("Set account fields in the global config.")
  .argument("<platform>", "x, linkedin, reddit, telegram, aegea, bluesky, mastodon, discord, or threads")
...
program
  ...
  .action((file, options) => run(() => browserPostCommand(file, options)));
...
program
  .command("preview")
  .description("Generate platform post text and save it for later publishing.")
  .argument("[markdown]", "Markdown input file")
  .option("-c, --config <path>", "Config file path")
  .option("-p, --profile <name>", "Profile name", "default")
  .option("-t, --target <id>", "Target id. Repeatable.", collect, [])
  ...
  .option("--input <markdown>", "Markdown input text")
  .option("--stdin", "Read Markdown input from stdin")
  .action((file, options) => run(() => previewCommand(file, options)));
